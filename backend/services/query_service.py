...
class QueryService:
    ...
    def process_query(self, message: str, organization: Dict, user_context: Dict = None) -> str:
        ...
        try:
            ...
            # Always translate non-English queries to English for RAG processing
            if detected_language != 'en' and confidence > 0.4:
                english_query, final_lang, final_confidence = self.translation_service.detect_and_translate_to_english(message)
                print(f"Translated '{message}' ({detected_language}) → '{english_query}' (en)")
            ...
            if not documents or query_type == "general":
                ...
                english_response = self._handle_general_query(english_query, organization, query_type, detected_language)
            else:
                # Handle document-specific queries with RAG
                english_response = self._handle_document_query(english_query, organization, documents, detected_language)
            ...
            if detected_language != 'en' and confidence > 0.4:
                ...
                print(f"Translated response back to {detected_language}")
            else:
            ...
            error_msg = self.language_service.get_localized_response('error', detected_language if 'detected_language' in locals() else 'en')
            return error_msg or "Sorry, there was an error processing your question. Please try again."
    
    def _handle_general_query(self, english_message: str, organization: Dict, query_type: str, user_language: str = "en") -> str:
        ...
        # Create contextual prompt with language awareness
    ...
    def _handle_document_query(self, english_message: str, organization: Dict, documents: List[Dict], user_language: str = "en") -> str:
        ...
        try:
            ...
            if not query_embedding:
                return self._fallback_keyword_search(english_message, organization, documents, organization_id, user_language)
            ...
            return self._fallback_keyword_search(english_message, organization, documents, organization.get("id"), user_language)
    
    def _fallback_keyword_search(self, english_message: str, organization: Dict, documents: List[Dict], organization_id: str = None, user_language: str = "en") -> str:
